...
#   define POOL_BOUNDARY nil
    ...
    pthread_t const thread; // 当前 Page 对应的线程
    // parent 和 child 为指向前驱和后继的指针
    // 自动释放池是一个以 Autoreleasepool 为节点的双向链表
    ...
    // 重写了 new 运算符，设置 page 的内存空间为 4096 ,实际就是虚拟内存页的大小
    ...
    // 设置内存起点为 this 长度为 SIZE 的访问限制：可读
    inline void protect() {
#if PROTECT_AUTORELEASEPOOL
    ...
    // 设置内存起点为 this 长度为 SIZE 的访问限制：可读、可写
    ...
    // sizeof(*this) 表示是 AutoreleasePoolPage 本身的大小
    // this + sizeof(*this) 表示的就是最低的地址
    id * begin() {
        return (id *) ((uint8_t *)this+sizeof(*this));
    }

    // this + SIZE 表示的就是最高的地址
    id * end() {
    ...
    {
        AutoreleasePoolPage *page = hotPage();
        if (page && !page->full()) {    // page 存在且没有满时，直接插入对象
            return page->add(obj);
        } else if (page) {      // 当 page 存在且已经满了时，扩展 page 节点并添加对象
            return autoreleaseFullPage(obj, page);
        } else {
            return autoreleaseNoPage(obj);  // 当 page 不存在时，就初始化一个
        }
    }
    ...
    id *autoreleaseFullPage(id obj, AutoreleasePoolPage *page)
    {
        ...
        // 从 child 方向找打未满的 page
        // 若找不到则创建一个新的 page 拼接到链表尾部
        do {
            if (page->child)
                page = page->child;
            else
                // 创建一个新的 page 并凭借到链表尾部
...
public:
    ...
    static inline void pop(void *token) 
    {
        ...
        if (DebugPoolAllocation  &&  page->empty()) {
            ...
            // 删除空的 child 链表节点，如果当前页对象超过一半，就保留下一个空节点
